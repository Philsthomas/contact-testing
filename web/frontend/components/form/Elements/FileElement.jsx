...
export function FileElement(props) {
  ...
  const { fields, submit } =
    useForm({
      fields: {
        ...
        className: useField({
          value: (elementDetails && elementDetails['css_class']) || ELEMENT_CLASS.FILE,
          validates: []
        }),
        fileType: useField({
          value: (elementDetails && elementDetails['file_type']) || '',
          validates: [],
        }),
        required: useField({
          value: required,
  ...
  return (
    <div className="elementBox">
      <Form onSubmit={submit}>
        <Layout>
          <Layout.Section>
            <h3 className="elementTitle">{t('Elements.fileUpload')}<div className="closeElement"><Tooltip content={t("Elements.closeElement")}><Button icon={XIcon} onClick={() => CloseElement(elementDetails, setEditElement, setElement)}></Button></Tooltip></div></h3>
            <Box className="elementContent">
              ...
              />
              <TextField
                label={t('Elements.fileType')}
                autoComplete="off"
                {...fields.fileType}
                helpText={<><div>{t('Elements.fileTypeNote')}</div><div>{t('Elements.fileTypeExample')}</div></>}
              />
