...
export function EmailElement(props) {
  ...
  const { fields, submit } =
    useForm({
      fields: {
        ...
        className: useField({
          value: (elementDetails && elementDetails['css_class']) || ELEMENT_CLASS.EMAIL,
          validates: []
        }),
        maxLength: useField({
          value: (elementDetails && elementDetails['max_length']) || '',
          validates: [(value) => {
  ...
  return (
    <div className="elementBox">
      <Form onSubmit={submit}>
        <Layout>
          <Layout.Section>
            <h3 className="elementTitle">{t('Elements.email')}<div className="closeElement"><Tooltip content={t("Elements.closeElement")}><Button icon={XIcon} onClick={() => CloseElement(elementDetails, setEditElement, setElement)}></Button></Tooltip></div></h3>
            <Box className="elementContent">
              ...
              <TextField
                ...
                autoComplete="off"
                disabled={(elementDetails && elementDetails['element_name'] == 'email') ? true : false}
                {...fields.elementName}
              />
              <TextField
