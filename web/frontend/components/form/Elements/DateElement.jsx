  ...
  const { fields, submit } =
    useForm({
      fields: {
        ...
        className: useField({
          value: (elementDetails && elementDetails['css_class']) || ELEMENT_CLASS.DATE,
          validates: []
        }),
        required: useField({
          value: required,
  ...
  return (
    <div className="elementBox">
      <Form onSubmit={submit}>
        <Layout>
          <Layout.Section>
            <h3 className="elementTitle">{t('Elements.date')}<div className="closeElement"><Tooltip content={t("Elements.closeElement")}><Button icon={XIcon} onClick={() => CloseElement(elementDetails, setEditElement, setElement)}></Button></Tooltip></div></h3>
